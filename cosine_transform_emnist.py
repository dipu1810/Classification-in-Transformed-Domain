...
#
mapping = {'letters':26, 'digits':10, 'mnist':10, 'byclass':62, 'bymerge':47, 'balanced':47}
...
def create_masks(x,y,percentage):
    ...
    for i in range(0, len(x)):
        F=dct(dct(x[i], axis=0, norm='ortho'), axis=1, norm='ortho')
        mag = np.abs(F)
        net_mag[y[i]]+=mag
    ...
    plt.title(f'DCT Coefficient Mask for {percentage*100}% coefficients retained', fontsize=15)
    plt.show()
    '''
    return union_mask
...
def compute_psnr(images,mask):
    ...
    for i in range(N):
        ...
        F=dct(dct(img, axis=0, norm='ortho'), axis=1, norm='ortho')
        masked_F = F * mask
        recon = idct(idct(masked_F, axis=0, norm='ortho'), axis=1, norm='ortho')
...
#Find out the distribution of y_train
...
percent_array=[0.001,0.005,0.01,0.05,0.1,0.5]
...
df.to_csv(rf'C:\Users\Mayukh\Documents\KGP\Sem8\Transformed Domain\EMNIST\experiment_results_dct_{category}.csv', index=False)
print("Saved")