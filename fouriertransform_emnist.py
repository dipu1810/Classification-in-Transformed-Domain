...
import time
import math
mapping = {'letters':62, 'digits':10, 'mnist':10, 'byclass':62, 'bymerge':47, 'balanced':47}
category = 'bymerge'  # options: byclass, bymerge, balanced, letters, digits, mnist
#
...
#
#input_path = r'C:\Users\Mayukh\Documents\KGP\Sem8\Transformed Domain\EMNIST\archive\emnist_source_files'x``
  # options: byclass, bymerge, balanced, letters, digits, mnist
training_images_filepath = rf"C:\Users\Mayukh\Documents\KGP\Sem8\Transformed Domain\EMNIST\archive\emnist_source_files\emnist-{category}-train-images-idx3-ubyte"
training_labels_filepath = rf"C:\Users\Mayukh\Documents\KGP\Sem8\Transformed Domain\EMNIST\archive\emnist_source_files\emnist-{category}-train-labels-idx1-ubyte"
test_images_filepath =rf"C:\Users\Mayukh\Documents\KGP\Sem8\Transformed Domain\EMNIST\archive\emnist_source_files\emnist-{category}-test-images-idx3-ubyte"
test_labels_filepath = rf"C:\Users\Mayukh\Documents\KGP\Sem8\Transformed Domain\EMNIST\archive\emnist_source_files\emnist-{category}-test-labels-idx1-ubyte"
...
def create_masks(x,y,percentage):
    net_mag=np.zeros((mapping[category],28,28))
    for i in range(0, len(x)):
        F=np.fft.fft2(x[i])
        mag = np.abs(F)
        net_mag[y[i]]+=mag

    binary_mask = np.zeros((mapping[category], 28, 28), dtype=int)

    for i in range(mapping[category]):
        N = net_mag[i].size
...
def compute_psnr(images,mask):
    ...
    for i in range(N):
        ...
        F = np.fft.fft2(img)
        masked_F = F * mask
        recon = np.fft.ifft2(masked_F).real

        mse = np.mean((img - recon) ** 2)
        psnr += 20 * np.log10(1.0 / np.sqrt(mse + 1e-8))
        percentage_error+=(np.sum(np.abs(masked_F**2)) / np.sum(np.abs(F**2)))

    return psnr / N, 1-percentage_error/N
...
def train_and_test(x_tr, y_tr, x_val, y_val, x_test, y_test,
    ...
    '''
    W =              model.layers[0].get_weights()[0]  # kernel of first Dense layer
    print(
        W.min(),
        W.max(),
        W.mean(),
    ...
    return train_duration,test_duration,model_size_mb, test_loss, test_acc,num_params,train_acc_history,val_acc_history
...
percent_array=[0.005,0.01,0.05,0.1,0.5]
...
percent_arr=[]
train_pp_arr=[]
percentage_error_arr=[]
test_pp_arr=[]
ITERATIONS=1
iterate_arr=[]
x_tr_tensor = tf.convert_to_tensor(x_tr, dtype=tf.float64)
x_val_tensor = tf.convert_to_tensor(x_val, dtype=tf.float64)
...
for i in range(ITERATIONS):
    ...
    for percent in percent_array:
        ...
        psnr, percentage_error = compute_psnr(x_tr,mask)
        psnr_arr.append(psnr)
        percentage_error_arr.append(percentage_error)
        ...
        for run in range(NUM_RUNS):
            ...
            train_dur,test_dur,model_size, test_loss, test_acc,num_params,train_acc_history,val_acc_history=train_and_test(x_tr_in,y_tr,x_val_in,y_val,x_test_in,y_test)
        ...
        print(f"Test Accuracy over {NUM_RUNS} runs: ",test_acc_runs)
        ...
        train_pp_arr.append(train_pp_end_time-train_pp_start_time)
        test_dur_arr.append(test_dur+test_pp_end_time-test_pp_start_time)
        test_pp_arr.append(test_pp_end_time-test_pp_start_time)
...
df = pd.DataFrame({
    ...
    'Train_Preprocessing_Time':train_pp_arr,
    'Test_Preprocessing_Time':test_pp_arr,
    'Coefficients': coefficients_arr,
    'Average PSNR': psnr_arr,
    'Average Percentage Error': percentage_error_arr,
...
#df.to_csv(rf'C:\Users\Mayukh\Documents\KGP\Sem8\Transformed Domain\EMNIST\experiment_results_fft_{category}.csv', index=False)
df.to_csv(rf'C:\Users\Mayukh\Documents\KGP\Sem8\Transformed Domain\EMNIST\experiment_results_fft2_{category}.csv', index=False)
print("Saved")
